...
public class SortOperator extends QueryOperator {
    ...
    public Run mergeSortedRuns(List<Run> runs) {
        ...
        for (int i = 0; i < runIterators.size(); ++i) {
            if (runIterators.get(i).hasNext()) {
                priorityQueue.add(new Pair<>(runIterators.get(i).next(), i));
            }
        }
        while (!priorityQueue.isEmpty()) {
            Pair<Record, Integer> pair = priorityQueue.poll();
            int index = pair.getSecond();
            sortedList.add(pair.getFirst());
            if (runIterators.get(index).hasNext()) {
                priorityQueue.add(new Pair<>(runIterators.get(index).next(), index));
            }
        }
        return makeRun(sortedList);
    }
    ...
    public List<Run> mergePass(List<Run> runs) {
        ...
        int index = 0;
        while (index + num <= runs.size()) {
            List<Run> runList = runs.subList(index, Math.min(runs.size(), index + num));
            res.add(mergeSortedRuns(runList));
            index += num;
        }
    ...
    public Run sort() {
        ...
        while (sourceIterator.hasNext()) {
            BacktrackingIterator<Record> recordIterator = getBlockIterator(sourceIterator, getSchema(), numBuffers);
            runs.add(sortRun(recordIterator));
        }
