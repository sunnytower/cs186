...
public class QueryPlan {
    ...
    public QueryOperator minCostSingleAccess(String table) {
        ...
        int expect = -1;
        List<Integer> indexColumn = getEligibleIndexColumns(table);
        for (Integer indexCol : indexColumn) {
            SelectPredicate p = selectPredicates.get(indexCol);
    ...
    public Map<Set<String>, QueryOperator> minCostJoins(
        ...
        for (Set<String> tableSet : prevMap.keySet()) {
            for (JoinPredicate predicate: this.joinPredicates) {
                ...
                Set<String> newTableSet = new HashSet<>(tableSet);
                if (tableSet.contains(leftTable) && !tableSet.contains(rightTable)) {
                    rightOp = findOpFromMap(pass1Map, rightTable);
                    leftOp = prevMap.get(tableSet);
                    newTableSet.add(rightTable);
                } else if (tableSet.contains(rightTable) && !tableSet.contains(leftTable)) {
                    leftOp = findOpFromMap(pass1Map, leftTable);
                    rightOp = prevMap.get(tableSet);
                    newTableSet.add(leftTable);
                } else {
                    continue;
                }
                QueryOperator bestOp = minCostJoinType(leftOp, rightOp, leftCol, rightCol);
                result.put(newTableSet, bestOp);
            }
        }
        return result;
    }

    private QueryOperator findOpFromMap(Map<Set<String>, QueryOperator> passMap, String table) {
        for (Set<String> tableSet : passMap.keySet()) {
            if (tableSet.contains(table)) {
                return passMap.get(tableSet);
            }
        }
        return null;
    }
    ...
    public Iterator<Record> execute() {
        ...
        // pass i
        Map<Set<String>, QueryOperator> resultMap = pass1Map;
        for (int i = 2; i <= tableNames.size(); ++i) {
            resultMap = minCostJoins(resultMap, pass1Map);
        }
        finalOperator = minCostOperator(resultMap);
        this.addGroupBy();
        this.addProject();
        this.addSort();
        this.addLimit();
