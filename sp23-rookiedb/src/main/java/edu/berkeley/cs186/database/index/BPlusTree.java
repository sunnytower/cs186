...
public class BPlusTree {
    ...
    public Iterator<RecordId> scanAll() {
        ...
        return new scanAllIterator();
    }

    private class scanAllIterator implements Iterator<RecordId> {
        private LeafNode currentLeaf;
        private Iterator<RecordId> currentLeafIter;
        private scanAllIterator() {
            currentLeaf = root.getLeftmostLeaf();
            currentLeafIter = currentLeaf.scanAll();
        }
        @Override
        public boolean hasNext() {
            if (!currentLeafIter.hasNext() && currentLeaf.getRightSibling().isPresent()) {
                currentLeaf = currentLeaf.getRightSibling().get();
                currentLeafIter = currentLeaf.scanAll();
            }
            return currentLeafIter.hasNext();
        }

        @Override
        public RecordId next() {
            if (!hasNext()) {
            throw new UnsupportedOperationException("no next value");
            }
            return currentLeafIter.next();
        }
    }
    ...
    public Iterator<RecordId> scanGreaterEqual(DataBox key) {
        ...
        return new scanGreaterEqualIterator(key);
    }

    private class scanGreaterEqualIterator implements Iterator<RecordId> {
        private LeafNode currentLeaf;
        private Iterator<RecordId> currentLeafIter;
        private scanGreaterEqualIterator(DataBox key) {
            currentLeaf = root.get(key);
            currentLeafIter = currentLeaf.scanGreaterEqual(key);
        }
        @Override
        public boolean hasNext() {
            if (!currentLeafIter.hasNext() && currentLeaf.getRightSibling().isPresent()) {
