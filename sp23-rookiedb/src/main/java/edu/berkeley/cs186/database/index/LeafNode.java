...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        if (keys.size() > 2 * d) {
            ...
            for (int i = 0; i < d + 1; i++) {
                keys.add(this.keys.remove(d));
                rids.add(this.rids.remove(d));
            }
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
            float fillFactor) {
        int logical2d = Math.round(metadata.getOrder() * 2 * fillFactor);
        while (data.hasNext() && keys.size() <= logical2d) {
            Pair<DataBox, RecordId> elem = data.next();
            keys.add(elem.getFirst());
            rids.add(elem.getSecond());
        }
        if (keys.size() <= logical2d) {
            sync();
            return Optional.empty();
        }
        ...
        List<DataBox> keys = new ArrayList<>();
        List<RecordId> rids = new ArrayList<>();
        keys.add(this.keys.remove(logical2d));
        rids.add(this.rids.remove(logical2d));
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        Optional<Long> rightSibling = sibPage > 0 ? Optional.of(sibPage) : Optional.empty();
        int n = buf.getInt();
        List<DataBox> keys = new ArrayList<>();
        List<RecordId> rids = new ArrayList<>();
        for (int i = 0; i < n; ++i) {
